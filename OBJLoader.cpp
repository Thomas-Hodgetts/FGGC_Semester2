...
MeshData OBJLoader::Load(char* filename, ID3D11Device* _pd3dDevice, bool invertTexCoords)
{
	...
	if(!binaryInFile.good())
	{
		...
		else
		{
			...
			XMFLOAT3 CentreOfGravity = XMFLOAT3();
			for (int i = 0; i < meshVertices.size(); i++)
			{
				CentreOfGravity.x + finalVerts[i].Pos.x;
				CentreOfGravity.y + finalVerts[i].Pos.y;
				CentreOfGravity.z + finalVerts[i].Pos.z;
			}

			CentreOfGravity.x / sizeof(finalVerts);
			CentreOfGravity.y / sizeof(finalVerts);
			CentreOfGravity.z / sizeof(finalVerts);

			meshData.Point = CentreOfGravity;
